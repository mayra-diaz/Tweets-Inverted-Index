...
class IndexHandler():
    ...
    def analyze_query(self, text, n):
        ...
        index_docs_weights = self.inverted_index.get_index_interseccion_with_query(list(query_tf.keys()))
        ...
        n = n if len(final) >= n else len(final)
        return self.inverted_index.get_tweets(list(final.keys())[:n])
        
    def cosine_distance(self, q, doc):
        q = np.log10(1 + q) 
        doc = np.log10(1 + doc)
        div = np.linalg.norm(q) * np.linalg.norm(doc)
        result = np.dot(q, doc)/div if div>0 else 0
        return result
